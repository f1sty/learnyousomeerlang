...
commit(S = #state{}) ->
  io:format("Transaction completed for ~s. Items sent are:~n~p,~n received "
            "are:~n~p.~nThis operation should have some atomic save in a "
            "database.~n",
            [S#state.name, S#state.ownitems, S#state.otheritems]).

priority(OwnPid, OtherPid) when OwnPid > OtherPid ->
  true;
priority(OwnPid, OtherPid) when OtherPid > OwnPid ->
  false.
...
unexpected(Msg, State) ->
  io:format("~p received unknown even ~p while in state ~p~n", [self(), Msg, State]).
